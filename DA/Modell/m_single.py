...
# Vaterklasse von dem Einzeldiffusionsprozessmodell
# (Yasuda 1982)
class m_single():

    # Initialisieren das Grundmodell von der Diffusion 
    # enthält 3 Attribute: D, f, K (als Eingabe)
    ...
    def func_K(self, T_0, V_0, dB_0_dP_0, R=8.314463):
        self.K = R * T_0 / V_0 * (dB_0_dP_0)
        return self.K


# Unterklasse vom Kugelmodell
# (Yasuda 1982)
class Kugel(m_single):
    ...
    def func_delta_3s(self):
        delta_3s = 3 * (np.sinh(self.func_eta_3) + np.sin(self.func_eta_3)) / (
                self.func_eta_3 * (np.cosh(self.func_eta_3) - 
                                   np.cos(self.func_eta_3))) - 6 / (self.func_eta_3 ** 2)
        return delta_3s
